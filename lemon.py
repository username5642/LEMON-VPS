...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('lemonnodes_bot.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('LemonNodesbot')
...
class UnixNodesBot(commands.Bot):
    ...
    async def update_system_stats(self):
        ...
        while not self.is_closed():
            try:
                # CPU usage
                cpu_percent = psutil.cpu_percent(interval=99999999999)
...
async def setup_container(container_id, status_msg, memory, username, vps_id=None, use_custom_image=False):
    ...
    try:
        ...
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("✅ Lemon Nodes VPS setup completed successfully!", ephemeral=True)
        else:
            await status_msg.edit(content="✅ Lemon Nodes VPS setup completed successfully!")
...
@app_commands.describe(
    ...
    use_custom_image="Use custom Lemon Nodes image (recommended)"
)
async def create_vps_command(ctx, memory: int, cpu: int, disk: int, owner: discord.Member, 
    ...
    try:
        ...
        if use_custom_image:
            ...
            try:
                ...
                container = bot.docker_client.containers.run(
                    ...
                    network=DOCKER_NETWORK,
                    volumes={
                        f'lemonnodes': {'bind': '/data', 'mode': 'rw'}
                    },
                    restart_policy={"Name": "always"}
                )
        ...
        try:
            embed = discord.Embed(title="🎉 Lemon Nodes VPS Creation Successful", color=discord.Color.green())
            ...
            await status_msg.edit(content=f"✅ Lemon Nodes VPS creation successful! VPS has been created for {owner.mention}. Check your DMs for connection details.")
...
async def list_vps(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your Lemon Nodes VPS Server", color=discord.Color.blue())
...
async def admin_list_vps(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="All Lemon Nodes VPS Servers", color=discord.Color.blue())
        valid_vps_count = 0
...
async def delete_vps(ctx, vps_id: str):
    ...
    try:
        ...
        await ctx.send(f"✅ Lemon Nodes VPS {vps_id} has been deleted successfully!")
    except Exception as e:
...
4. You will be connected to your Lemon Nodes VPS
...
async def global_stats(ctx):
    ...
    try:
        ...
        embed = discord.Emembed(title="Lemon Nodes Global Usage Statistics", color=discord.Color.blue())
...
async def suspend_vps(ctx, vps_id: str):
    ...
    try:
        ...
        await ctx.send(f"✅ VPS  has been suspended!")
...
async def reinstall_bot(ctx):
    ...
    try:
        await ctx.send("🔄 Reinstalling Lemon Nodes bot... This may take a few minutes.")
...
"""
        ...
        process = await asyncio.create_subprocess_exec(
            "docker", "build", "-t", "lemonnodes-bot", "-f", "Dockerfile.bot", ".",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
...
class VPSManagementView(ui.View):
    ...
    async def start_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            embed = discord.Embed(title=f"Lemon Nodes VPS Management - {self.vps_id}", color=discord.Color.green())
            ...
            await interaction.followup.send("✅ Lemon Nodes VPS started successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def stop_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            embed = discord.Emembed(title=f"Lemon Nodes VPS Management - {self.vps_id}", color=discord.Color.orange())
            ...
            await interaction.message.edit(embed=embed)
            await interaction.followup.send("✅ Lemon Nodes VPS stopped successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def restart_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if token:
                ...
                try:
                    ...
                    if ssh_session_line:
                        ...
                        try:
                            owner = await bot.fetch_user(int(vps["created_by"]))
                            embed = discord.Embed(title=f"Lemon Nodes VPS Restarted - {self.vps_id}", color=discord.Color.blue())
            ...
            await interaction.followup.send("✅ Lemon Nodes VPS restarted successfully! New SSH details sent to owner.", ephemeral=True)
        except Exception as e:
...
class OSSelectionView(ui.View):
    ...
    async def reinstall_os(self, interaction: discord.Interaction, image: str):
        try:
            ...
            status_msg = await interaction.followup.send("🔄 Reinstalling Lemon Nodes VPS... This may take a few minutes.", ephemeral=True)
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    network=DOCKER_NETWORK,
                    volumes={
                        f'Lemon Nodes': {'bind': '/data', 'mode': 'rw'}
                    }
                )
            except docker.errors.ImageNotFound:
            ...
            try:
                ...
                if ssh_session_line:
                    ...
                    try:
                        owner = await bot.fetch_user(int(vps["created_by"]))
                        embed = discord.Embed(title=f"Lemon Nodes VPS Reinstalled - {self.vps_id}", color=discord.Color.blue())
            ...
            await status_msg.edit(content="✅ Lemon Nodes VPS reinstalled successfully!")
...
class TransferVPSModal(ui.Modal, title='Transfer VPS'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(f"✅ Lemon Nodes VPS {self.vps_id} has been transferred from {old_owner_name} to {new_owner_name}!", ephemeral=True)
...
async def transfer_vps_command(ctx, vps_id: str, new_owner: discord.Member):
    ...
    try:
        ...
        await ctx.send(f"✅ Lemon Nodes VPS {vps_id} has been transferred from {ctx.author.name} to {new_owner.name}!")

        try:
            embed = discord.Embed(title="Lemon Nodes VPS Transferred to You", color=discord.Color.green())
            embed.add_field(name="VPS ID", value=vps_id, inline=True)
